...
*注：开启 `SAVE_QUESTION: true` 后，提交题目时会将完整的题目、所有选项和正确答案存为单独 JSON 到 `save/` 目录下（方便作为新的公开题源）。*
获取：
![alt text](image.png)
...
4. **自动答题及题库更新**
   ...
   - **没找到的题目自动猜与补充**：如果遇到**题库中没有的题目**，脚本会随机提交一个选项。无论后续系统判错还是判对，只要有确切的正确答案，就会将其自动补充进 `answer/补充题库.json` 文件内。
